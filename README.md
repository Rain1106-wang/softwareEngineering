...
- `get_vars`：从代码的 AST（抽象语法树）中提取变量名。
- `get_vars_heuristics`：当 AST 解析失败时，启发式地从代码中提取变量名。
...
- `get_wordpos`：根据词性标记获取对应的WordNet词性，用于词性还原和词干提取。根据给定的词性标签（tag）返回对应的WordNet词性。
...
- `filter_invachar`：根据正则表达式模式去除句子中的非法字符。使用正则表达式匹配非常用字符，并将其替换为空格。去除输入字符串中的非常用符号，以防止解析时出现错误。
- `filter_all_invachar`：去除字符串中的非常用符号并规范化字符。
- `python_code_parse`：解析Python代码行，去除非常用字符并转换为tokens列表。对Python代码进行解析，提取代码中的标记化的代码列表。根据代码的语法规则和标点符号，将代码分割为单词，并进行标准化和词干提取等操作。
- `python_query_parse`：解析Python查询字符串，去除非常用字符并进行自然语言处理。对自然语言查询进行解析，提取查询中的单词列表。首先去除非法字符和括号，然后进行去冗处理、分词、标记化、词性标注、词形还原和词干提取等操作。
- `python_context_parse`：解析Python上下文代码，去除非常用字符并进行自然语言处理。对自然语言上下文进行解析，提取上下文中的单词列表。首先去除非法字符，然后进行去冗处理、分词、标记化、词性标注、词形还原和词干提取等操作。
...
- `tokenizeRegex`：使用正则表达式扫描器对输入的字符串 s 进行扫描，并返回扫描结果。
- `sanitizeSql`：对输入的 SQL 语句进行预处理，确保其格式符合解析要求。
- `__hash__`：计算 SqlangParser 对象的哈希值，基于其 token 列表。
- `__init__`：初始化 SqlangParser 对象，解析和处理 SQL 语句。
- `getTokens`：获取解析树中的所有 token，并将其展平为一个列表。
- `removeWhitespaces`：去除SQL语句中的空白符号。移除 SQL 解析树中的空白 token。
...
- `__str__`：返回 SQL 解析树中 token 的字符串表示。
- `parseSql`：获取 SQL 解析树中所有 token 的字符串表示。
- `revert_abbrev`：将输入字符串中的英文缩写还原为完整形式。
...
- `filter_all_invachar`：去除输入字符串中的所有非常用符号。用于过滤句子中的非常用符号。
- `filter_part_invachar`：去除输入字符串中的部分非常用符号。用于过滤代码中的非常用符号。
